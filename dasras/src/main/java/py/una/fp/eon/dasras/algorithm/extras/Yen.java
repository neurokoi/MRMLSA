...
public class Yen<V, E> extends KShortestPathAlgorithm<V, E> {
	...
	protected List<List<E>> getShortestPathsIntern(final V source, final V target, int k) {
		...
		if (dijkstra.getPathWeight(source, target) == Double.POSITIVE_INFINITY)
			return found_paths;

		// Add Dijkstra solution, the first shortest path.
		found_paths.add(dijkstra.getPath(source, target).getEdgeList());
		...
		while (found_paths.size() < k) {
			...
			for (int i = 0; i < maxIndex; i++) {
				...
				V deviation = head.isEmpty() ? source : graph.getEdgeTarget(head.get(i - 1));

				// 1. Block edges.
				Graph<V, E> blocked = blockFilter(head, deviation, found_paths);

				// 2. Get shortest path in graph with blocked edges.
				blockedDijkstra = new DijkstraShortestPath<V, E>(blocked);
				// , nev

				double dist = blockedDijkstra.getPathWeight(deviation, target);
				if (dist == Double.POSITIVE_INFINITY)
					continue;

				GraphPath<V, E> tail = blockedDijkstra.getPath(deviation, target);

				// 3. Combine head and tail into new path.
				List<E> candidate = new ArrayList<E>(i + tail.getLength());
				candidate.addAll(head);
				candidate.addAll(tail.getEdgeList());
	...
	private Graph<V, E> blockFilter(List<E> head, V deviation, List<List<E>> foundPaths) {
		...
		for (E e : head)
			for (E e2 : graph.edgesOf(graph.getEdgeSource(e)))
				blocked.add(e2);
		...
		for (List<E> path : foundPaths)
			if (path.size() > head.size() && ListUtils.isEqualList(path.subList(0, head.size()), head))
				for (E e : path)
					if (graph.getEdgeSource(e).equals(deviation)) {
						blocked.add(e);
		...
		EdgePredicateFilter<V, E> filter = new EdgePredicateFilter<V, E>(predicate);

		return filter.apply(graph);
	}
}
